...
class DirectionalTakerStrategy(Strategy):
    """
    Directional taking strategy (optional).
    
    Crosses the spread when edge is very high.
    """
    
    def __init__(
        self,
        min_take_edge_ticks: int = 5,
        max_take_size: float = 20.0,
    ):
        """
        Initialize the strategy.
        
        Args:
            min_take_edge_ticks: Minimum edge to take
            max_take_size: Maximum taker size
        """
        self._min_take_edge_ticks = min_take_edge_ticks
        self._max_take_size = max_take_size
        self._enabled = False  # Disabled by default
    
    @property
    def name(self) -> str:
        return "DirectionalTaker"
    ...
    def on_tick(self, ctx: DecisionContext) -> StrategyIntent:
        """Generate taker actions if edge is sufficient."""
        intent = StrategyIntent()
        ...
        min_edge = self._min_take_edge_ticks * tick_size
        expires_at_ms = ctx.now_ms + 1000  # 1 second TTL for taker
        
        # Take ask if fair >> ask (buy)
        if fair - market_ask > min_edge:
            intent.take_actions.append(DesiredOrder(
                side=Side.BUY,
                price=market_ask,  # Cross the spread
                size=self._max_take_size,
                purpose=OrderPurpose.TAKE,
                expires_at_ms=expires_at_ms,
                token_id=token_id,
            ))
        
        # Take bid if bid >> fair (sell)
        if market_bid - fair > min_edge:
            intent.take_actions.append(DesiredOrder(
                side=Side.SELL,
                price=market_bid,  # Cross the spread
                size=self._max_take_size,
                purpose=OrderPurpose.TAKE,
                expires_at_ms=expires_at_ms,
                token_id=token_id,
